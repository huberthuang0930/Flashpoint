...
import type { IAPData, IAPSection } from "../lib/types";
 ...
 */
function parseIAPSections(text: string): IAPSection[] {
 ...
 */
function extractAirTactics(content: string): string[] {
 ...
 */
function extractMetadata(filename: string, text: string): Partial<IAPData> {
 ...
 */
function extractTacticalLessons(text: string, sections: IAPSection[]): string[] {
 ...
 */
function inferFuelType(text: string): "grass" | "brush" | "mixed" | "chaparral" | undefined {
...
/**
 * Process a single IAP PDF file
 */
async function processIAPFile(filename: string): Promise<IAPData | null> {
  ...
  try {
    ...
    console.log(`   ✓ Extracted ${sections.length} sections, ${tacticalLessons.length} lessons`);
    return iapData;
...
async function main() {
  ...
  for (const filename of pdfFiles) {
    const iapData = await processIAPFile(filename);
    if (iapData) {
